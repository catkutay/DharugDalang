...
response.menu = [  ['Home', False, URL(r=request, c='plugin_wiki', f='page', args='index')],
             ['Accept Regos', False, URL(r=request, c='appadmin', f='update', args="db/auth_user/registration_key/pending")],

                [T('Nominations'), False, URL(r=request, c='appadmin', f='update', args="db/Nomination_Form/")],
                [T('Submission'), False, URL(r=request, c='appadmin', f='update', args="db/Content_Form/")],
#  [T('design'), False, URL('admin', 'default', 'design',  args=[request.application])],
                 [T('db'), False, URL(r=request, c='appadmin', f='index')], [T('state'), False,
                 URL(r=request, c='appadmin', f='state')],
...
def insert():
	...
	tables=db[table]
	 ...
	 form = SQLFORM(dblanguage[table], ignore_rw=ignore_rw)
	 tables=dblanguage(table)
    if form.accepts(request.vars, session):
        response.flash = T('new record inserted')
    return dict(form=form,table=tables)
...
def update():
    ...
    if keyed:
		...
		key = request.args(2)
                value= str(request.args(3))
                record = db(db[table][key] == value).select().first()
    else:
        try:
    ...
    if not record:
        ...
        redirect(URL(r=request,c='appadmin', f='select', args=request.args[:1],
                     vars=dict(query=qry)))
    ...
    if form.accepts(request.vars, session):
        ...
        if table=='auth_user' and record:
             if value == 'pending':  #updating rego to accept
                if form.vars.RIMO_Staff:
                        auth.add_membership(3,record.id)
                if form.vars.RIBS_Staff:
                        auth.add_membership(4,record.id)
   	     qry = query_by_table_type(table, db)
             redirect(URL(r=request, c='appadmin', f='update',
                 args="db/auth_user/registration_key/pending"))
